...
namespace Toboggan
{
    public class Controller : MonoBehaviour
    {
        ...
        void Start()
        {
            FinishManager.Register(this.transform);
        }
        ...
        void RotateToGroundNormal()
        {
            ...
            if (Physics.Raycast(transform.position, downwardCastVector, out rayHit, 5000))
            {
                normalIndex = (normalIndex + 1) % 3;
                normal[normalIndex] = rayHit.normal;
                average = ((normal[0] + normal[1] + normal[2]) / 3);

                if (transform.up.y - rayHit.normal.y < 0.05f) { return; }
                //transform.rotation = Quaternion.FromToRotation(transform.up, average);// * transform.rotation;
                Quaternion wantedRotation = Quaternion.LookRotation(average);
                transform.rotation = Quaternion.Lerp(transform.rotation, wantedRotation, Time.time * 5000);
            }
