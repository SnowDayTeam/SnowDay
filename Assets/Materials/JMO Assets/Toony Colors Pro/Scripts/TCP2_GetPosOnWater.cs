...
public class TCP2_GetPosOnWater : MonoBehaviour
{
	...
	public bool followWaterNormal = false;
	...
	void LateUpdate()
	{
		...
		{
			var worldPosition = GetPositionOnWater(this.transform.position);
		...
		{
			this.transform.rotation = Quaternion.FromToRotation(upAxis, GetNormalOnWater(this.transform.position));
			this.transform.Rotate(postRotation, Space.Self);
		}
	}
