...
public class TCP2_Demo_Camera : MonoBehaviour
{
	...
	void Update()
	{
		...
		//Pan
		Pivot.Translate(panAcceleration * Time.deltaTime, this.transform);
		this.transform.Translate(panAcceleration * Time.deltaTime, this.transform);

		//Zoom
		float scrollWheel = Input.GetAxis("Mouse ScrollWheel");
		zoomAcceleration += scrollWheel * ZoomStrg;
		zoomAcceleration = Mathf.Clamp(zoomAcceleration, -ZoomClamp, ZoomClamp);
		zoomDistance = Vector3.Distance(this.transform.position, Pivot.position);
