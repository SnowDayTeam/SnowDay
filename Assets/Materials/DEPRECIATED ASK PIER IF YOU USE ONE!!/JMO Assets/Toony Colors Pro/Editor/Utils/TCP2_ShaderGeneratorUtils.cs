...
public static class TCP2_ShaderGeneratorUtils
{
	...
	static public Shader Compile(TCP2_Config config, Shader existingShader, TCP2_ShaderGenerator.ShaderGeneratorTemplate template, bool showProgressBar = true, bool overwritePrompt = true)
	{
	...
	static private string GenerateShaderSource(this TCP2_Config config, TCP2_ShaderGenerator.ShaderGeneratorTemplate template, Shader existingShader = null)
	{
		...
		//Include path
		string include = GetIncludePrefix(config) + GetIncludeRelativePath(config, existingShader).TrimEnd('/');
		...
		for(int i = 0; i < templateLines.Length; i++)
		{
			...
			if(line.StartsWith("#"))
			{
				...
				if (line.StartsWith("#KEYWORDS"))
				{
					while (i < templateLines.Length)
					{
						...
						string error = ProcessKeywords(templateLines[i], config, ref features, ref flags, ref keywords, ref i, ref depth, ref stack, ref done);
			...
			if(line.Contains("///"))
			{
				string error = ProcessCondition(line, ref features, ref i, ref depth, ref stack, ref done);
				if(!string.IsNullOrEmpty(error))
				{
					return error;
				}
	...
	static private string ProcessKeywords(string line, TCP2_Config config, ref List<string> features, ref List<string> flags, ref Dictionary<string,string> keywords, ref int i, ref int depth, ref List<bool> stack, ref List<bool> done)
	{
		if (line.Contains("///"))
		{
	...
	static private string ProcessCondition(string line, ref List<string> features, ref int i, ref int depth, ref List<bool> stack, ref List<bool> done)
	{
	...
	static private bool EvaluateExpression( int lineNumber, List<string> features, params string[] conditions )
	{
		...
		try
		{
			TCP2_ExpressionParser.ExpressionLeaf.EvaluateFunction evalFunc = ( string s ) => HasEntry(features, s);
	...
	static private Shader SaveShader(TCP2_Config config, Shader existingShader, string sourceCode, bool overwritePrompt, bool modifiedPrompt)
	{
		...
		string fullPath = systemPath + filename + ".shader";
		bool overwrite = true;
		...
		if(overwrite)
		{
			...
			//Write file to disk
			File.WriteAllText(fullPath, sourceCode, System.Text.Encoding.UTF8);
	...
	static public bool HasEntry( List<string> list, string entry )
	{
		return list.Contains(entry);
	}
	static public bool HasAnyEntries(List<string> list, params string[] entries)
	{
	...
	static public bool HasAllEntries( List<string> list, params string[] entries )
	{
		bool hasAll = true;
		foreach (string str in entries)
	...
	static public void ToggleEntry( List<string> list, string entry, bool toggle )
	{
