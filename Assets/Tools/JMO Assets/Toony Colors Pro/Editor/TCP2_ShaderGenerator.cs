...
public partial class TCP2_ShaderGenerator : EditorWindow
{
	...
	static private List<string> sOpenedFoldouts = new List<string>();
	...
	void OnEnable()
	{
		...
		if(allTemplates != null && allTemplates.Length > 0)
		{
			...
			List<object[]> menuItemsSorted = new List<object[]>();
	...
	void OnGUI()
	{
		...
		*/
		...
		if (TCP2_ShaderGeneratorUtils.CustomOutputDir)
		{
			...
			{
				string outputPath = TCP2_Utils.OpenFolderPanel_ProjectPath("Choose custom output directory for TCP2 generated shaders");
	...
	private void SaveUserPrefs()
	{
		...
		string openedFoldouts = string.Join(",", sOpenedFoldouts.ToArray());
	...
	private void ReloadUserShaders()
	{
		...
		for(int i = 0; i < mUserShaders.Length; i++)
		{
