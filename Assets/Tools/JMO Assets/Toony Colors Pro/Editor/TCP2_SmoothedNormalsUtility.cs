...
public class TCP2_SmoothedNormalsUtility : EditorWindow
{
	...
	void OnGUI()
	{
		...
		*/
		...
		using(new EditorGUI.DisabledScope(!mCustomDirectory))
		{
			...
			{
				string outputPath = TCP2_Utils.OpenFolderPanel_ProjectPath("Choose custom output directory for generated smoothed meshes");
	...
	private Mesh CreateSmoothedMeshAsset(SelectedMesh originalMesh)
	{
		...
		string rootPath = mCustomDirectory ? Application.dataPath + "/" + mCustomDirectoryPath + "/" : TCP2_Utils.FindReadmePath() + OUTPUT_FOLDER;
		...
		string newAssetName = originalMeshName + " " + MESH_SUFFIX + ".asset";
